...
import type { PlayerController } from '../spotify/types';
...
export function PlayerControls({ player, isPlaying, position, duration, onLogout }: Props) {
  ...
  const toggleFullscreen = useCallback(() => {
    ...
    if (!document.fullscreenElement) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ((document as any).exitFullscreen ?? (document as any).webkitExitFullscreen)?.call(document).catch(() => {});
    }
  }, []);
  ...
  return (
    <div className="player-controls">
      ...
      <div className="controls-row">
        ...
        <button className="ctrl-btn" onClick={() => player?.nextTrack()} aria-label="Next">
          ⏭
        </button>
        <button
